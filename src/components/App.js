...
export default class App extends React.Component {
		...
		$hands.each((i, elHand) => {
			...
			if (hand) {
				...
				let palmPos		= hand.palmPosition,
					palmLeft	= this._map(palmPos[0], LEAP_MIN_X, LEAP_MAX_X, 0, this.$grid.width()),
					palmTop		= this._map(palmPos[1], LEAP_MIN_Y, LEAP_MAX_Y, 0, this.$grid.height());
				...
				$fingers.each((j, elFinger) => {
					...
					if (finger) {
						...
						$finger.css({
							"left": this._map(fingerPos[0], LEAP_MIN_X, LEAP_MAX_X, 0, this.$grid.width()),
							"top": this._map(fingerPos[1], LEAP_MIN_Y, LEAP_MAX_Y, 0, this.$grid.height())
						});
	...
	_updateGrid(hands) {
		...
		const litCells	= hands.map((hand, i) => {
			return {
				col: Math.floor(this._map(hand.palmPosition[0], LEAP_MIN_X, LEAP_MAX_X, 0, GRID_COLS)),
				row: Math.floor(this._map(hand.palmPosition[1], LEAP_MIN_Y, LEAP_MAX_Y, 0, GRID_ROWS))
			};
		});
